...
- Los puntos **rojos y azules** representan las observaciones reales:
  - Azul: el individuo eligió automóvil (`y = 1`)
  - Rojo: el individuo eligió otro medio (`y = 0`)

- La **línea verde** representa la curva ajustada por máxima verosimilitud usando `glm()`:
  - Es la forma estándar de estimar una regresión logística.
  - Maximiza la log-verosimilitud.

- La **línea morada discontinua** representa la curva ajustada por mínimos cuadrados usando `nls()`:
  - Minimiza el error cuadrático entre los valores observados y los predichos por la función logística.
  - Aunque similar, puede diferir en los coeficientes y la forma exacta de la curva.

Ambas curvas deben seguir una forma sigmoidea, pero pueden diferir ligeramente según el método de ajuste utilizado.
